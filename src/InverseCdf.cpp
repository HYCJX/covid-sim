 ...
 * @return void
 */
void InverseCdf::set_neg_log(float start_value)
{
	cdf_values_[CDF_RES] = start_value;
	for (int i = 0; i < CDF_RES; i++)
		cdf_values_[i] = -log(1 - ((float)i) / CDF_RES);
}
 ...
 * @return void
 */
void InverseCdf::assign_exponent(float value)
{
	for (int i = 0; i <= CDF_RES; i++)
	{
 ...
 */
unsigned short int InverseCdf::choose(float Mean, int tn, float timesteps_per_day)
{
