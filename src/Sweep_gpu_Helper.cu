...
__device__ double sinx_GPU[DEGREES_PER_TURN + 1];
__device__ double cosx_GPU[DEGREES_PER_TURN + 1];
__device__ double asin2sqx_GPU[1001];
...
__device__ int32_t Xcg1_GPU[MAX_NUM_THREADS * CACHE_LINE_SIZE];
__device__ int32_t Xcg2_GPU[MAX_NUM_THREADS * CACHE_LINE_SIZE];
...
//    __shared__ double s5_cache[threadsPerBlock];
//    int cacheIndex = threadIdx.x;
//    s5_cache[cacheIndex] = 0;
//    __syncthreads();
...
//    FILE *stderr_shared = stderr;
    ...
    while (b < P_GPU->NCP) {
        Cell *c = CellLookup_GPU + b; // select Cell given by index b
        for (int j = 0; j < c->I; j++) {
            ...
            int fct = ((P_GPU->DoDigitalContactTracing) &&
                       ...
                       && (t < AdUnits_GPU[Mcells_GPU[si->mcell].adunit].DigitalContactTracingTimeStart +
                               P_GPU->DigitalContactTracingPolicyDuration) &&
                       (Hosts_GPU[ci].digitalContactTracingUser ==
            ...
            if (hbeta > 0) {
                ...
                if ((Households_GPU[si->hh].nh > 1) && (!si->Travelling)) {
                    ...
                    if (f) {
                        s3 *= P_GPU->PlaceCloseHouseholdRelContact;
                    }/* NumPCD++;}*/ //// if people in your household are absent from places, person si/ci is more infectious to them, as they spend more time at home.
                    ...
                    for (int i3 = l; i3 < m; i3++) //// loop over all people in household (note goes from l to m - 1)
                    {
                        if ((Hosts_GPU[i3].inf == InfStat_Susceptible) &&
                            (!Hosts_GPU[i3]
                                    .Travelling)) //// if people in household uninfected/susceptible and not travelling
                        {
                            ...
                            if (ranf_mt_GPU(tn) < s) {
                                ...
                                for (int i = 0; i < 32; i++) {
                                    if ((j % 32) == i) {
                                        Inf_Locks_GPU[cq].lock();
                                        if ((StateT_GPU[tn].n_queue[cq] <
                                             P_GPU->InfQueuePeakLength)) //(Hosts[i3].infector==-1)&&
                                        {
                                            if ((P_GPU->FalsePositiveRate > 0) &&
                                                (ranf_mt_GPU(tn) < P_GPU->FalsePositiveRate))
                                                StateT_GPU[tn].inf_queue[cq][StateT_GPU[tn].n_queue[cq]++] =
                                                        {-1, i3, -1};
                                            else {

                                                // ** infect household member i3 **
                                                data->infectors_modified[i3] =
                                                        ci; //// assign person ci as infector of person i3
                            ...
                            if ((k != P_GPU->HotelPlaceType) && (!si->Travelling)) {
                                ...
                                if (n > 0) {
                                    ...
                                    SampleWithoutReplacement_GPU(tn, n,
                                                                 Places_GPU[k][l].group_size[i2],
                                                                 SamplingQueue_GPU); //// changes thread-specific SamplingQueue.
                                }
                                ...
                                for (int m = 0; m < n; m++) {
                                    ...
                                    s = CalcPlaceSusc_GPU(i3, k, ts, ci, tn, Hosts_GPU, HostsQuarantine_GPU, Mcells_GPU,
                                                          P_GPU);
                                    ...
                                    if ((fct) && (Hosts_GPU[i3].digitalContactTracingUser) && (ci != i3) &&
                                        ...
                                        if ((Hosts_GPU[ci].ncontacts < P_GPU->MaxDigitalContactsToTrace) &&
                                            (ranf_mt_GPU(tn) < s6)) {
                                            data->ncontracts_increased[ci] = 1; //add to number of contacts made
                                            int ad = Mcells_GPU[Hosts_GPU[i3].mcell].adunit;
                                            for (int i = 0; i < 32; i++) {
                                                if ((j % 32) == i) {
                                                    Dct_Locks_GPU[ad].lock();
                                                    if ((StateT_GPU[tn].ndct_queue[ad] < AdUnits_GPU[ad].n)) {
                                                        ...
                                                        StateT_GPU[tn].dct_queue[ad][StateT_GPU[tn].ndct_queue[ad]++] =
                                                                {i3, ci,
                                                                 ts};
                                                    } else {
                                                         ...
                                                         P_GPU->InfQueuePeakLength)) //(Hosts[i3].infector==-1)&&
                                                    {
                                                        ...
                                                        if ((P_GPU->FalsePositiveRate > 0) &&
                                                            ...
                                                            short int infect_type =
                                                                    2 + k +
                                                                    INFECT_TYPE_MASK *
                                                                    (1 + si->infect_type / INFECT_TYPE_MASK);
                                                            StateT_GPU[tn].inf_queue[cq][StateT_GPU[tn].n_queue[cq]++] =
                                                                    {
                                                                            ci,
                                                                            i3,
...
//                                                ad);
                                                ...
                                                if ((j % 32) == i) {
                                                         ...
                                                         P_GPU->InfQueuePeakLength))//(Hosts[i3].infector==-1)&&
                                                    {
                                                        ...
                                                        if ((P_GPU->FalsePositiveRate > 0) &&
                                                            ...
                                                            short int infect_type =
                                                                    2 + k + NUM_PLACE_TYPES + INFECT_TYPE_MASK * (1 +
                                                                                                                  si->infect_type /
                                                                                                                  INFECT_TYPE_MASK);
            ...
            if (sbeta > 0) {
                ...
                if (P_GPU->DoPlaces) {
                    ...
                    for (int i3 = 0; (i3 < P_GPU->PlaceTypeNum) && (!f); i3++) {
                        if (si->PlaceLinks[i3] >= 0) //// if person has a link to place of type i3...
                        {
                            ...
                            f = PLACE_CLOSED_GPU(i3,
                                                 si->PlaceLinks[i3]); //// find out if that place of type i3 is closed.
                        }
                    }
                }// if doing places
                        ...
                        ci))) //// if place is closed and person is absent then adjust the spatial infectiousness (similar logic to household infectiousness: place closure affects spatial infectiousness
                {
                    ...
                    /* NumPCD++; */
//                s5_cache[cacheIndex] = s2;
//                StateT_GPU[tn].cell_inf[j] = (float) -s5_cache[cacheIndex];
                } else {
//                s5_cache[cacheIndex] = s2;
//                StateT_GPU[tn].cell_inf[j] = (float) -s5_cache[cacheIndex];
                }
...
//    while (i != 0) {
//        if (cacheIndex < i) {
//            s5_cache[cacheIndex] += s5_cache[cacheIndex + i];
//        }
//        __syncthreads();
//        i /= 2;
//    }
//    if (cacheIndex == 0) {
//        Inf_Locks_GPU[0].lock();
//        data->s5 += s5_cache[0];
//        Inf_Locks_GPU[0].unlock();
//    }
}